...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	...
	echo $'exec geth --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --http.vhosts "*"' >> wallet.sh
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	stats := &walletInfos{
		datadir:  infos.volumes["/root/.cetd"],
		nodePort: nodePort,
		rpcPort:  rpcPort,
		webHost:  host,
		webPort:  webPort,
